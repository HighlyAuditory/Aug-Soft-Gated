...
from data.utils import get_parsing_label_tensor, \
    get_label_tensor, get_thetas_tensor, get_thetas_affgrid_tensor, get_parsing_tensor, get_image_tensor
...
def test_paper_img(tag='nips_cvpr'):
    ...
    for i in xrange(len(pairs)):
        ...
        fake_image, a_image_tensor, b_image_tensor, b_label_show_tensor, fake_b_parsing_tensor = get_test_result(a_jpg_path, b_jpg_path, model, opt)

        test_list = [
            ('b_label_show', tensor2im(b_label_show_tensor)),
            ('a_image', tensor2im(a_image_tensor)),
            ('real_b_image', tensor2im(b_image_tensor)),
            ('fake_b_parsing', tensor2im(parsingim_2_tensor(fake_b_parsing_tensor, opt=opt, parsing_label_nc=opt.parsing_label_nc)[0])),
