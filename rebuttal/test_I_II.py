...
from data.utils import get_thetas_affgrid_tensor_by_json
from data.utils import get_parsing_label_tensor
...
def main():
    web_dir = os.path.join(opt.results_dir, opt.name, '%s_%s_%s' % (opt.phase, opt.which_epoch, "I_and_II"))
    ...
    for i, data in enumerate(dataset):
        ...
        a_parsing_tensor_RGB_numpy = util.parsing2im(util.label_2_onhot(a_parsing_tensor[0], parsing_label_nc=opt.parsing_label_nc))
        ...
        theta_aff_tensor, theta_tps_tensor, theta_aff_tps_tensor = get_thetas_affgrid_tensor_by_json(affTnf, tpsTnf, theta_json)
