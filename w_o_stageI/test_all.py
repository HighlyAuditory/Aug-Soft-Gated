...
def main():
    ...
    for i, data in enumerate(dataset):
        ...
        input_tensor = torch.cat([a_image_tensor, b_image_tensor, b_label_tensor], dim=1)
        input_var = Variable(input_tensor.type(torch.cuda.FloatTensor))

        model.eval()
        fake_b = model.inference(input_var)
        show_image_tensor = torch.cat((a_image_tensor, b_label_show_tensor, b_image_tensor, fake_b.data[0:1, :, :, :].cpu()), dim=3)
